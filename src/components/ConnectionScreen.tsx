...
import { DEFAULT_ROS_IP } from '../constants';
import { RobotIcon } from './icons/RobotIcon';
import { SignalIcon } from './icons/SignalIcon';

interface ConnectionScreenProps {
    onConnect: (ip: string, version: RosVersion) => void;
...
const ConnectionScreen: React.FC<ConnectionScreenProps> = ({ onConnect, status }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!isConnecting) {
            onConnect(ipAddress, rosVersion);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <div className="flex justify-center mb-4">
                            <div className="relative flex w-full max-w-xs p-1 bg-gray-800 rounded-lg">
                                <button type="button" onClick={() => setRosVersion('ros1')} className={`relative w-1/2 rounded-md py-2 text-sm font-medium transition-colors ${rosVersion === 'ros1' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>ROS 1</button>
                                <button type="button" onClick={() => setRosVersion('ros2')} className={`relative w-1/2 rounded-md py-2 text-sm font-medium transition-colors ${rosVersion === 'ros2' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>ROS 2</button>
                                <div className="absolute top-1 left-1 bottom-1 w-[calc(50%-4px)] bg-blue-600 rounded-md transition-transform duration-300 ease-in-out" style={{ transform: rosVersion === 'ros2' ? 'translateX(100%)' : 'translateX(0)' }}/>
                            </div>
                        </div>
