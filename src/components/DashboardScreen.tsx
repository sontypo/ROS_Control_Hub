...
import { Topic, RosMessage } from '../types';
...
const DashboardScreen: React.FC<{ onDisconnect: () => void }> = ({ onDisconnect }) => {
    ...
    const [availableTopics, setAvailableTopics] = useState<Omit<Topic, 'data'>[]>([]);
    const [showTopicModal, setShowTopicModal] = useState(false);
    const [showPublishModal, setShowPublishModal] = useState(false);
    
    useEffect(() => {
        // Fetch available topics from the robot on connect
        rosService?.getAvailableTopics()
            .then(topics => {
                setAvailableTopics(topics);
                // Optionally auto-subscribe to /odom if it exists
                const odomTopic = topics.find(t => t.name === '/odom');
                if (odomTopic) {
                    subscribeToTopic(odomTopic);
                }
            })
    ...
    const subscribeToTopic = useCallback((topic: Omit<Topic, 'data'>) => {
        ...
        rosService.subscribe(topic.name, topic.type, (data) => handleDataUpdate(topic.name, data));
    ...
    const availableToSubscribe = availableTopics.filter(at => !subscribedTopics.some(st => st.name === at.name));
